function setPosAndLED(valueStore){
	...
	for (var y=0; y<10; y++) {
		...
		brightness = y / 9;
		phaseRad += toRadians(((y / 9) * 90) + timeInSeconds);
		for (var x=0; x<10; x++) {
			...
			//console.log("freqRad=",freqRad);
			//console.log("phaseRad=", phaseRad);
			//console.log("sineSample=",sineSample);
			valueStore.pos[x][y] = bias + (amplitude * Math.sin(x * TWOPI * (sineSample) + phaseRad));
